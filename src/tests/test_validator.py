...
class StubParser:
    ...
    def first_segment(self) -> dict:
        return {"label": "intro", **self.SEGMENTS["intro"]}

    def last_segment(self) -> dict:
        return {"label": "chorus", **self.SEGMENTS["chorus"]}
...
def test_validate_all_valid(parser):
    results = validate(parser, Clip("intro"), Clip("verse"), Clip("chorus"))
    assert results == []
...
def test_validate_loop_valid(parser):
    results = validate(parser, Clip("intro"), Loop("chorus", times=3))
    assert results == []
