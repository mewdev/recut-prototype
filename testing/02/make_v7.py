...
OUT   = f'{STEMS}/chicago-cut-v7.mp3'
...
# --- bass + vox: taper out at cut point ---
...
# --- drums_reverb: 400Hz high-pass → 0.7 wet hall reverb ---
# active only between cut_samp and sweep_samp (phrase A repeat zone)
drums_rev = chain(
    drums, sr,
    (eq,     dict(cutoff=400, type="high")),
    (reverb, dict(wetness=0.7, reverb_type="hall")),
)
drums_rev[:, :cut_samp] = 0.0
drums_rev[:, cut_samp : cut_samp + taper_samp] *= np.linspace(0.0, 1.0, taper_samp)
# taper OUT at sweep point — don't let reverbed drums enter the creative exit
drums_rev[:, sweep_samp - taper_samp : sweep_samp] *= np.linspace(1.0, 0.0, taper_samp)
drums_rev[:, sweep_samp:] = 0.0

# --- single continuous mix ---
mix = combine_stems(piano, bass, vox, drums_dry, drums_rev)

# --- creative exit: piano-only slice, no drums_rev ---
# extract piano from sweep point, apply chain
