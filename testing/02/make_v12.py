...
OUT   = f'{STEMS}/chicago-cut-v12.mp3'

TRIM_START    = 61.56
DRUM_CUT_ABS  = 196.68   # 2:15 — bass + vox cut
SWEEP_ABS     = 210.90   # 2:29 — piano exit starts
CLIP_END_ABS  = 231.00
LAST_BAR_ABS  = 209.135  # 2:27.57 — last 4 kicks begin their sweep

CLIP_DUR  = CLIP_END_ABS - TRIM_START
cut_rel   = DRUM_CUT_ABS - TRIM_START   # 135.12s
sweep_rel = SWEEP_ABS    - TRIM_START   # 149.34s
bar_rel   = LAST_BAR_ABS - TRIM_START   # 147.575s
bar_dur   = SWEEP_ABS    - LAST_BAR_ABS # 1.765s
...
fi_samp    = int(0.5 * sr)   # piano_end fade-in

cut_samp   = int(cut_rel   * sr)
sweep_samp = int(sweep_rel * sr)
bar_samp   = int(bar_rel   * sr)
...
# --- drums: last 4 kicks (last bar) get filter_sweep + fade, then silent ---
last_bar = drums[:, bar_samp:sweep_samp].copy()
last_bar = chain(
    last_bar, sr,
    (filter_sweep, dict(direction="close", duration=bar_dur,
                        freq_ceil=8000.0, freq_floor=80.0, curve=1.0)),
    (fade,         dict(vol_start=1.0, vol_end=0.0)),
)
drums[:, bar_samp:sweep_samp] = last_bar
...
# fade IN over 0.5s — softens reverb onset
