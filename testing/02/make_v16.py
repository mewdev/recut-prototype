...
OUT   = f'{STEMS}/chicago-cut-v16.mp3'

TRIM_START        = 61.56
DRUM_CUT_ABS      = 196.68   # 2:15 — bass+vox cut, drums begin fading
DRUM_FADE_END_ABS = 207.34   # 2:25.78 — drums reach zero
...
# --- drums: fade 1.0→0.0 from 2:15 to 2:25.78, then silent ---
drums[:, cut_samp:drum_fade_samp] = fade(
    drums[:, cut_samp:drum_fade_samp].copy(), sr, vol_start=1.0, vol_end=0.0
)
drums[:, drum_fade_samp:] = 0.0

# --- piano dry: full until sweep_samp ---
# apply gentle 4kHz lowpass in exposed zone (drum_fade_samp → sweep_samp)
# to soften cello/piano attacks that were masked by kick
piano_dry = piano.copy()
piano_dry[:, drum_fade_samp:sweep_samp] = eq(
    piano_dry[:, drum_fade_samp:sweep_samp].copy(), sr, cutoff=4000, type="low"
)
# short taper at sweep_samp
